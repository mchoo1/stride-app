...
const divider: React.CSSProperties = { height: 1, background: '#E5E9F2', margin: '20px 0' };
...
export default function RegisterPage() {
  ...
  // ── Step 0 validation: format-only (no Firebase call)
  // NOTE: fetchSignInMethodsForEmail was removed — it was deprecated and returned
  // empty arrays for all emails in recent Firebase versions, allowing duplicate
  ...
  async function finish() {
    ...
    try {
      ...
      if (code === 'auth/email-already-in-use') {
        // Reset to Step 0 so the error shows on the Account screen with the Sign in link
        setStep(0);
        ...
        setError(`Something went wrong. Please try again. (${code || String(err)})`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F7F8FB' }}>
      ...
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', background: '#fff', borderBottom: '1px solid #E5E9F2' }}>
        <button onClick={() => step === 0 ? router.push('/') : back()} style={{ background: 'none', border: 'none', fontSize: 14, fontWeight: 600, color: '#8B95A7', cursor: 'pointer' }}>
          ← Back
        </button>
        <StrideWordmark height={26} />
        <span style={{ fontSize: 12, fontWeight: 600, color: '#C8D0DC' }}>{step + 1} / {STEPS.length}</span>
      </div>
      ...
      <div style={{ flex: 1, maxWidth: 480, width: '100%', margin: '0 auto', padding: '28px 20px 48px', overflowY: 'auto' }}>
        ...
        {step === 0 && (
          <>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: '#0F1B2D', margin: '0 0 6px' }}>Create your account</h2>
        ...
        {step === 1 && (
          <>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: '#0F1B2D', margin: '0 0 6px' }}>Goal &amp; preferences</h2>
                ...
                return (
                  <button key={g.key} onClick={() => update('goalType', g.key)} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ width: 22, height: 22, borderRadius: '50%', flexShrink: 0, border: `2px solid ${sel ? g.color : '#E5E9F2'}`, background: sel ? g.color : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <p style={{ fontSize: 12, color: '#8B95A7', marginBottom: 14, marginTop: -4 }}>Used to filter food recommendations. Select all that apply.</p>
        ...
        {step === 2 && (
          <>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: '#0F1B2D', margin: '0 0 6px' }}>Your body stats</h2>
            <p style={{ fontSize: 14, color: '#8B95A7', marginBottom: 24 }}>Used to calculate your personalised calorie target.</p>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="number" className="form-input" value={rawNums.age} onChange={e => setRaw('age', e.target.value)} placeholder="27" min="10" max="100" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#5B6576', marginBottom: 6 }}>Height (cm)</label>
                <input type="number" className="form-input" value={rawNums.height} onChange={e => setRaw('height', e.target.value)} placeholder="170" min="100" max="250" />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#5B6576', marginBottom: 6 }}>Current weight (kg)</label>
                <input type="number" className="form-input" value={rawNums.cw} onChange={e => setRaw('cw', e.target.value)} placeholder="75" min="20" max="300" step="0.1" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#5B6576', marginBottom: 6 }}>Target weight (kg)</label>
                <input type="number" className="form-input" value={rawNums.tw} onChange={e => setRaw('tw', e.target.value)} placeholder="65" min="20" max="300" step="0.1" />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {ACTIVITY_LEVELS.map(a => {
                ...
                return (
                  <button key={a.key} onClick={() => update('activityLevel', a.key as UserProfile['activityLevel'])} style={{
                    ...
                    <div style={{ width: 20, height: 20, borderRadius: '50%', flexShrink: 0, border: `2px solid ${sel ? '#1E7F5C' : '#E5E9F2'}`, background: sel ? '#1E7F5C' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', paddingTop: 20 }}>
            <div style={{ width: 88, height: 88, borderRadius: '50%', background: 'rgba(30,127,92,0.10)', border: '2px solid rgba(30,127,92,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
              <StrideIcon size={52} />
            </div>
            ...
            {/* Summary card */}
            <div style={{ width: '100%', borderRadius: 20, padding: 16, marginBottom: 24, background: '#fff', border: '1px solid #E5E9F2', boxShadow: '0 2px 12px rgba(15,27,45,0.06)' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#C8D0DC', letterSpacing: '0.08em', marginBottom: 14 }}>YOUR SETUP</div>
            ...
            {/* Error only shows here for non-auth errors (network, operation-not-allowed, etc.)
                auth/email-already-in-use resets to step 0 before reaching this point */}
            {error && (
              <div style={{ width: '100%', borderRadius: 12, padding: '12px 14px', marginBottom: 16, background: '#fff5f5', border: '1px solid #fecaca' }}>
