...
export default function EatPage() {
  ...
  const searchResults = useMemo(() => {
    ...
    for (const rest of SG_RESTAURANTS) {
      ...
      if (matchesName) {
        // Only show popular items when matching by restaurant name — avoids
        // flooding results with the entire menu (60+ items for McDonald's etc.)
        const popularItems = rest.menu.filter(i => !i.isSetMeal && i.isPopular);
        const toShow = popularItems.length > 0 ? popularItems : rest.menu.filter(i => !i.isSetMeal).slice(0, 8);
    ...
    const combined = [...byItemName, ...byRestaurantName];
    // Sort by proximity: items with known distance first, unknowns last
    combined.sort((a, b) => (a.distKm ?? 99) - (b.distKm ?? 99));
    return combined;
  }, [query, distLookup]);
  ...
  const restaurantList = useMemo(() => {
    ...
    // Sort by proximity when GPS is available
    if (locState === 'granted') {
      list = [...list].sort((a, b) => (distLookup.get(a.id) ?? 99) - (distLookup.get(b.id) ?? 99));
    }
    return list;
  }, [query, diningOption, priceFilter, filterDietFlags, outletFilter, locState, distLookup]);
